...
func (s *server) Start(ctx context.Context) error {
	...
	s.start.Do(func() {
		...
		if err := s.graphDB.Start(ctx); err != nil {
			startErr = err
			return
		}
...
func (s *server) InboundPeerConnected(ctx context.Context, conn net.Conn) {
	...
	case ErrPeerNotConnected:
		...
		s.peerConnected(ctx, conn, nil, true, access)
...
func (s *server) OutboundPeerConnected(ctx context.Context,
	...
	case ErrPeerNotConnected:
		// We were unable to locate an existing connection with the
		// target peer, proceed to connect.
		s.peerConnected(ctx, conn, connReq, false, access)
...
func (s *server) peerConnected(ctx context.Context, conn net.Conn,
	connReq *connmgr.ConnReq, inbound bool, access peerAccessStatus) {
