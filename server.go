...
func (s *server) Start(ctx context.Context) error {
	...
	s.start.Do(func() {
		...
		if shouldPeerBootstrap(s.cfg) {
			...
			s.wg.Add(1)
			go s.peerBootstrapper(ctx, defaultMinPeers, bootstrappers)
		} else {
...
func (s *server) establishPersistentConnections() error {
	...
	err = s.graphDB.ForEachNodeChannelTx(nil, sourceNode.PubKeyBytes,
...
// closed.
func (s *server) connectToPeer(ctx context.Context,
	addr *lnwire.NetAddress, errChan chan<- error, timeout time.Duration) {
