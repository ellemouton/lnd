		...
		for {
			...
			case topChange, ok := <-graphSub.TopologyChanges:
				...
				for _, update := range topChange.NodeUpdates {
					...
					// our persistentPeers.
					s.mu.RLock()
					ok := s.persistentPeerMgr.
						IsPersistentPeer(pubKeyStr)
					s.mu.RUnlock()
					if !ok {
						continue
					}
...
func (s *server) establishPersistentConnections() error {
	...
	for pubStr, nodeAddr := range nodeAddrsMap {
		...
		// persistent connection with. We set the perm to false in
...
func (s *server) peerTerminationWatcher(p *peer.Brontide, ready chan struct{}) {
	...
	// Add any missing addresses for this peer to the
	// PersistentPeerManager.
	for _, addr := range addrs {
		s.persistentPeerMgr.AddPeerAddresses(
			pubStr, &lnwire.NetAddress{
