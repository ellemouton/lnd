...
func testWatchtowerSessionManagement(ht *lntest.HarnessTest) {
	...
	// number of backups backed up to the tower.
	assertNumBackups := func(expected int) {
		err = wait.NoError(func() error {
	...
	ht.MineBlocksAndAssertNumTxes(sessionCloseRange+5, 1)
	...
	// To make sure that the force closed channel gets properly cleaned up,
	// mine enough blocks so that both the local and remote nodes can
	// sweep their funds.
	ht.MineBlocksAndAssertNumTxes(node.DefaultCSV, 2)
}
