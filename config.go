...
func ValidateConfig(cfg Config, interceptor signal.Interceptor, fileParser,
	...
	// For backwards compatibility, the cfg.Bitcoin.Active option is still
	// exposed to the user, but it now must be set to true since no other
	// chains are supported.
	if !cfg.Bitcoin.Active {
		return nil, fmt.Errorf("bitcoin.active must be set to 1 (true)")
	}
