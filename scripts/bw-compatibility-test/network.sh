...
# wait_for_node waits for the given node in the cluster to start.
...
function setup_network() {
  wait_for node_started alice bob charlie dave
...
# wait_for is a generic function to wait for any condition for the given nodes.
wait_for() {
    if [[ $# -lt 2 ]]; then
        echo "❌ Error: do_for requires at least 2 arguments (function and nodes)"
        echo "Usage: wait_for <function> [node1] [node2] [node3]..."
        return 1
    fi
    ...
    for node in "${nodes[@]}"; do
        echo -n "⌛ Waiting for $node to satisfy '$func'"

        # Silence the function's output but still check its return value
        while ! "$func" "$node" >/dev/null 2>&1; do
            echo -n "."
            sleep 0.5
        done

        echo
        echo "✅ $node satisfied '$func'"
    done

    echo "🏎️ All nodes satisfied '$func'!"
}

# node_started checks if a node has started.
node_started() {
    local node="$1"

    if $node getinfo 2>/dev/null | grep -q identity_pubkey; then
        echo "✅ $node has started"
        return 0
    else
        echo "❌ $node has NOT started yet"
        return 1
    fi
}
