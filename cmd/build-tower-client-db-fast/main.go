...
func (t *things) do() error {
	...
	// populate a bunch of in mem acked updates(ie create a session of map[seqNum]*backupID)
	// then, only once the session is full do we write the acked updates to the db.
...
func (t *things) ackUpdates(backupID wtdb.BackupID) error {
	...
	if t.activeSession.seqNum == 1024 {
		// Persist it and then also create a new one
		err := t.db.AckAll(&t.activeSession.id, t.activeSession.acks)
		if err != nil {
			return err
		}
		t.createNewSession()
	...
	t.activeSession.acks[t.activeSession.seqNum] = &backupID
...
func (t *things) createNewSession() error {
	...
	t.activeSession = &session{
		id:     sessionID,
		seqNum: 0,
		acks:   make(map[uint16]*wtdb.BackupID),
	}
