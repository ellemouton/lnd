...
func copyTower(t *testing.T, tower *Tower) *Tower {
	addrs := getAllAddresses(tower.Addresses)
...
func getAllAddresses(iter *AddressIterator) []net.Addr {
	iter.Reset()
	addrs := []net.Addr{iter.Peek(false)}
	for {
		addr, err := iter.Next(false)
		if err != nil {
			break
		}
...
func assertNextCandidate(t *testing.T, i TowerCandidateIterator, c *Tower) {
	...
	require.Equal(t, getAllAddresses(tower.Addresses),
		getAllAddresses(c.Addresses))
}
...
func TestTowerCandidateIterator(t *testing.T) {
	...
	// the tower.
	firstTower := towers[0]
	firstTowerAddr := firstTower.Addresses.Peek(false)
