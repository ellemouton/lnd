...
func getClientSessions(db DB, keyRing ECDHKeyRing, forTower *wtdb.TowerID,
	...
	sessions, err := db.ListClientSessions(forTower, sessionFilter, opts...)
	if err != nil {
		return nil, err
	}
...
func (c *TowerClient) handleStaleTower(msg *staleTowerMsg) error {
	...
	sessions, err := c.cfg.DB.ListClientSessions(&tower.ID, nil)
	if err != nil {
...
// if they match the policy of the client. If activeOnly is set, then only
// active sessions will be returned.
