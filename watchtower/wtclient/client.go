...
type Client interface {
	...
	// the client.
	RegisteredTowers(...wtdb.ClientSessionOption) ([]*RegisteredTower,
		error)

	// LookupTower retrieves a registered watchtower through its public key.
	LookupTower(*btcec.PublicKey, ...wtdb.ClientSessionOption) (
...
func New(config *Config) (*TowerClient, error) {
	...
	*cfg = *config
	...
	candidateSessions, err := getClientSessions(
		...
		wtdb.WithCommittedUpdates(), wtdb.WithAckedUpdateSummary(),
	)
	if err != nil {
		return nil, err
	}
...
func (c *TowerClient) buildHighestCommitHeights() {
	...
	for _, s := range c.candidateSessions {
		...
		// updates.
		for chanID, commitHeight := range s.MaxChanCommitHeights {
			height, ok := chanCommitHeights[chanID]
			if !ok || commitHeight > height {
				chanCommitHeights[chanID] = commitHeight
			}
		}
	}
...
// client.
func (c *TowerClient) RegisteredTowers(opts ...wtdb.ClientSessionOption) (
	[]*RegisteredTower, error) {
...
func (c *TowerClient) LookupTower(pubKey *btcec.PublicKey,
	opts ...wtdb.ClientSessionOption) (*RegisteredTower, error) {
