...
type TowerClient struct {
	...
	terminateSession chan *terminateSessMsg
...
func New(config *Config) (*TowerClient, error) {
	...
	*cfg = *config
	...
	c := &TowerClient{
		...
		terminateSession:     make(chan *terminateSessMsg),
...
func (c *TowerClient) backupDispatcher() {
	...
	for {
		...
		awaitSession:
			...
			// only this if the tower is not currently being used
...
func (c *TowerClient) TerminateSession(id *wtdb.SessionID) error {
	...
	select {
	case c.terminateSession <- &terminateSessMsg{
		id:      id,
		errChan: errChan,
	}:
	case <-c.pipeline.quit:
...
func (c *TowerClient) handleTerminateSession(msg *terminateSessMsg) error {
	id := *msg.id
...
func (c *TowerClient) handleDeactivateTower(msg *deactivateTowerMsg) error {
	...
	return c.cfg.DB.DeactivateTower(msg.pubKey)
}
