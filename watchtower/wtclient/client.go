...
const (
	...
	// DefaultMaxQueueBuffer is the maximum number of items to be held in
	// the in-memory queue.
	DefaultMaxQueueBuffer = 1000
)
...
func New(config *Config) (*TowerClient, error) {
	...
	*cfg = *config
	...
	queue, err := NewDiskOverflowQueue[*wtdb.BackupID](
		cfg.DB, cfg.MaxTasksInMemQueue, plog,
	)
	if err != nil {
		return nil, err
	}
...
func (c *TowerClient) Stop() error {
	...
	c.stopped.Do(func() {
		...
		// 2. Shutdown all active session queues in parallel. These will
		...
		// 3. Shutdown the backup queue, which will prevent any further
		// updates from being accepted.
		if err := c.pipeline.Stop(); err != nil {
			returnErr = err
		}

		// 4. Once the backup queue has shutdown, wait for the main
