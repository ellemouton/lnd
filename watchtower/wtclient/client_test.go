...
func (c *mockChannel) sendPayment(t *testing.T, amt lnwire.MilliSatoshi) {
	...
	require.Falsef(t, c.localBalance < amt, "insufficient funds to send, "+
		"need: %v, have: %v", amt, c.localBalance)
...
func (c *mockChannel) receivePayment(t *testing.T, amt lnwire.MilliSatoshi) {
	...
	require.Falsef(t, c.remoteBalance < amt, "insufficient funds to recv, "+
		"need: %v, have: %v", amt, c.remoteBalance)
...
func (h *testHarness) stopServer() {
	...
	if err := h.server.Stop(); err != nil {
		h.t.Fatalf("unable to stop wtserver: %v", err)
	}
}
...
func (h *testHarness) waitServerUpdates(hints []blob.BreachHint,
	...
	for {
		...
		case <-time.After(time.Second):
			...
			require.NoErrorf(h.t, err, "unable to query for "+
				"hints: %v", err)
