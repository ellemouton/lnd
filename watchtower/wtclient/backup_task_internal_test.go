...
func testBackupTask(t *testing.T, test backupTaskTest) {
	...
	// key was inserted.
	if hasToRemote {
		require.Equal(t, jKit.CommitToRemotePubKey[:], expToRemotePK)
	} else {
	...
	if hasToLocal {
		require.Falsef(t, emptyToLocalSig, "to-local signature should "+
			"not be empty")
	} else {
	...
	if hasToRemote {
		require.Falsef(t, emptyToRemoteSig, "to-remote signature "+
			"should not be empty")
	} else {
