	...
	return kvdb.Update(c.db, func(tx kvdb.RwTx) error {
		...
		if towerIndexBkt == nil {
			return fmt.Errorf("no entry in the tower-to-session "+
				"index found for tower ID %x", sess.TowerID)
		}
		...
		if ackRanges == nil {
			...
			return fmt.Errorf("no acked-updates found for "+
				"session %s", id)
		}
...
// is considered closable only if all the following points are true: 1) It has
// no un-acked updates, 2) It is exhausted (ie it can't accept any more
// updates), 3) All the channels that it has acked updates for are closed.
