...
// DeactivateTower sets the given tower's status to inactive. It also sets the
// status of any session with the tower to inactive. CreateTower can be used to
// reactivate the tower and all its sessions again.
...
func isSessionClosable(sessionsBkt, chanDetailsBkt, chanIDIndexBkt kvdb.RBucket,
	...
	if errors.Is(err, errSessionHasUnackedUpdates) && !isTerminal {
		return false, nil
	} else if err != nil {
		return false, err
	}
