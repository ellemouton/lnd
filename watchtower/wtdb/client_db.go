...
var (
	...
	// cSessionRogueUpdateCount is a sub-bucket of cSessionBkt storing the
	// number of rogue updates that were backed up using the session. Rogue
	// updates are updates for channels that have been closed already at
...
func (c *ClientDB) AckUpdate(id *SessionID, seqNum uint16,
	...
	return kvdb.Update(c.db, func(tx kvdb.RwTx) error {
		...
		if errors.Is(err, ErrChannelNotRegistered) {
			...
			if sessionAckRanges != nil {
				return fmt.Errorf("session(%s) has and "+
