...
func MigrateTowerToSessionIndex(tx kvdb.RwTx) error {
	...
	for towerID, sessions := range entries {
		for sessionID := range sessions {
			err := addIndex(indexBkt, towerID, sessionID)
			if err != nil {
				return err
			}
		}
...
// addIndex adds the given towerID-sessionID pair to the given bucket. The
// tower ID is used as a sub-bucket name and the session ID is used as a key
// within the sub-bucket. A value of []byte{1} is used for each session ID key.
func addIndex(bucket kvdb.RwBucket, towerID TowerID,
	sessionID SessionID) error {

	towerBkt, err := bucket.CreateBucketIfNotExists(towerID.Bytes())
	if err != nil {
		return err
	}
