...
// This index is a mapping from db-assigned ID (8 bytes) to real channel ID
// (32 bytes). This mapping will allow us to persist channel pointers with fewer
// bytes in the future.
func MigrateChannelIDIndex(tx kvdb.RwTx) error {
	...
	return chanDetailsBkt.ForEach(func(chanID, _ []byte) error {
		...
		return chanDetails.Put(cChanDBID, newIndex[:])
	})
}
