...
// This index is a mapping from db-assigned ID (8 bytes) to real session ID
// (33 bytes). This mapping will allow us to persist session pointers with fewer
// bytes in the future.
func MigrateSessionIDIndex(tx kvdb.RwTx) error {
	...
	return sessionsBkt.ForEach(func(sessionID, _ []byte) error {
		...
		sessBkt := sessionsBkt.NestedReadWriteBucket(sessionID)
		if sessBkt == nil {
		...
		sessionBodyBytes := sessBkt.Get(cSessionBody)
		...
		return sessBkt.Put(cSessionDBID, newIndex[:])
	})
}
