...
func NewRangeIndex(ranges map[uint64]uint64) (*RangeIndex, error) {
	index := &RangeIndex{
...
func (a *RangeIndex) Add(newHeight uint64, kv KVStore) error {
	...
	err := applyKVChanges(kv, kvStoreChanges)
	if err != nil {
		return err
	}
...
func applyKVChanges(kv KVStore, changes *kvChanges) error {
	...
	if changes.deleteKVKey != nil {
		var del [8]byte
		byteOrder.PutUint64(del[:], *changes.deleteKVKey)
	...
	byteOrder.PutUint64(start[:], changes.key)
	byteOrder.PutUint64(end[:], changes.value)
...
func (a *RangeIndex) applyArrayChanges(changes *arrayChanges) {
	...
	if changes.indexToDelete != nil {
		if *changes.indexToDelete == len(a.set) {
			a.set = a.set[:*changes.indexToDelete]
		} else {
			a.set = append(
