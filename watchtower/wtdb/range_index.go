...
// numRanges returns the number of ranges the set is tracking.
func (a *RangeIndex) numRanges() int {
	a.mu.Lock()
	defer a.mu.Unlock()
...
// getRangeMax returns the maximum height in the range starting with the given
// number.
func (a *RangeIndex) getRangeMax(h uint64) uint64 {
	a.mu.Lock()
	defer a.mu.Unlock()
...
// Revert can be called to cancel a RangeIndexTx.
func (a *RangeIndexTx) Revert() {
	a.mu.Unlock()
}
