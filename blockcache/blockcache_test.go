...
func TestBlockCacheGetBlock(t *testing.T) {
	...
	require.Len(t, bc.blockMtxs.mutexMap, 0)
}

// TestBlockCacheMutexs is used to test that concurrent calls to GetBlock with
// the same block hash does not result in multiple calls to the chain backend.
// In other words this tests the blockMtxs.
func TestBlockCacheMutexs(t *testing.T) {
	mc := newMockChain()
	getBlockImpl := mc.GetBlock
