...
WHERE c.version = sqlc.arg(version)
  AND (
       (cp1.block_height >= sqlc.arg(start_height) AND cp1.block_height < sqlc.arg(end_height))
       OR
       (cp2.block_height >= sqlc.arg(start_height) AND cp2.block_height < sqlc.arg(end_height))
  )
...
-- name: GetChannelBySCIDPreferHighestVersionWithPolicies :one
SELECT
    sqlc.embed(c),
    sqlc.embed(n1),
    sqlc.embed(n2),
...
-- name: GetChannelByOutpointPreferHighestVersionWithPolicies :one
SELECT
    sqlc.embed(c),
...
FROM graph_channels c
    ...
    LEFT JOIN graph_channel_policies cp2
        ON cp2.channel_id = c.id AND cp2.node_id = c.node_id_2 AND cp2.version = c.version
WHERE c.outpoint = @outpoint
ORDER BY c.version DESC
LIMIT 1;

-- name: GetVersionsBySCID :many
SELECT version
FROM graph_channels
WHERE scid = @scid
ORDER BY version;

-- name: GetVersionsByOutpoint :many
SELECT version
FROM graph_channels
