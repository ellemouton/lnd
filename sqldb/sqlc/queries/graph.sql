...
WHERE graph_nodes.version = @version
  AND COALESCE(block_height, last_update) >= @start_time
  AND COALESCE(block_height, last_update) <= @end_time
  ...
  AND (
    ...
    COALESCE(block_height, last_update) >
        COALESCE(sqlc.narg('last_update'), -1)
    OR 
...
ORDER BY COALESCE(block_height, last_update) ASC, pub_key ASC
...
WHERE c.version = @version
  AND (
       (COALESCE(cp1.block_height, cp1.last_update) >= @start_time
           AND COALESCE(cp1.block_height, cp1.last_update) < @end_time)
       OR
       (COALESCE(cp2.block_height, cp2.last_update) >= @start_time
           AND COALESCE(cp2.block_height, cp2.last_update) < @end_time)
  )
