...
INSERT INTO channels (
    version, channel_id, node_id_1, node_id_2,
    outpoint, capacity
) VALUES (
    $1, $2, $3, $4, $5, $6
)
...
-- name: GetChannelByChannelIDAndVersion :one
SELECT * FROM channels
WHERE channel_id = $1 AND version = $2;

-- name: GetChannelIDByOutpoint :one
...
INSERT INTO zombie_channels (channel_id, version, node_key_1, node_key_2)
VALUES ($1, $2, $3, $4)
ON CONFLICT (channel_id, version)
    DO UPDATE SET
...
-- name: GetV1DisabledChannelIDs :many
SELECT c.channel_id
FROM channels c
...
-- name: GetPublicV1ChannelsByChannelID :many
SELECT c.*
FROM channels c
         JOIN v1_channel_proofs p ON p.channel_id = c.id
WHERE c.channel_id >= sqlc.arg(start_channel_id)
  AND c.channel_id < sqlc.arg(end_channel_id);

-- name: GetChannelsByChannelIDRange :many
SELECT *
FROM channels
WHERE channel_id >= sqlc.arg(start_channel_id)
  AND channel_id < sqlc.arg(end_channel_id);
