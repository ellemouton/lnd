...
FROM graph_channels c
         ...
         LEFT JOIN graph_channel_policies cp2
                   ON cp2.channel_id = c.id AND cp2.node_id = c.node_id_2 AND cp2.version = c.version
WHERE c.version = $1
AND (c.node_id_1 IN (/*SLICE:node_ids*/?)
     OR c.node_id_2 IN (/*SLICE:node_ids*/?))
`
...
func (q *Queries) ListChannelsForNodeIDs(ctx context.Context, arg ListChannelsForNodeIDsParams) ([]ListChannelsForNodeIDsRow, error) {
	...
	if len(arg.NodeIds) > 0 {
		...
		query = strings.Replace(query, "/*SLICE:node_ids*/?", makeQueryParams(len(queryParams), len(arg.NodeIds)), 2)
	} else {
