...
func createEdge(height, txIndex uint32, txPosition uint16, outPointIndex uint32,
	node1, node2 *LightningNode) (models.ChannelEdgeInfo1,
...
func assertEdgeWithPolicyInCache(t *testing.T, g *ChannelGraph,
	e *models.ChannelEdgeInfo1, p *models.ChannelEdgePolicy1,
	policy1 bool) {
...
func TestEdgePolicyMissingMaxHtcl(t *testing.T) {
	...
	encodingInfo, err := encodingInfoFromEdgePolicy(edge1)
	require.NoError(t, err)

	var b bytes.Buffer
	err = serializeChanEdgePolicy(&b, encodingInfo, to)
	if err != nil {
	...
	var b2 bytes.Buffer
	err = serializeChanEdgePolicy(&b2, encodingInfo, to)
	if err != nil {
		t.Fatalf("unable to serialize policy")
	}
