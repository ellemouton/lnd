...
func createEdge2(height, txIndex uint32, txPosition uint16,
	outPointIndex uint32, node1, node2 *LightningNode) (*ChannelEdgeInfo2,
	...
	edgeInfo := ChannelEdgeInfo2{
		...
		AuthProof: &ChannelAuthProof2{
			SchnorrSigBytes: testSchnorrSigStr,
		},
	}

	copy(edgeInfo.NodeID1[:], node1Pub.SerializeCompressed())
	copy(edgeInfo.NodeID2[:], node2Pub.SerializeCompressed())

	var btc1, btc2 [33]byte
	copy(btc1[:], node1Pub.SerializeCompressed())
	copy(btc2[:], node2Pub.SerializeCompressed())
	edgeInfo.BitcoinKey1 = &btc1
	edgeInfo.BitcoinKey2 = &btc2
...
func assertEdgeInfoEqual(t *testing.T, e1, e2 models.ChannelEdgeInfo) {
	...
	case *ChannelEdgeInfo1:
		edge2, ok := e2.(*ChannelEdgeInfo1)
		require.True(t, ok)

		assertEdgeInfo1Equal(t, edge1, edge2)
	case *ChannelEdgeInfo2:
		edge2, ok := e2.(*ChannelEdgeInfo2)
		require.True(t, ok)

		assertEdgeInfo2Equal(t, edge1, edge2)
	default:
		t.Fatalf("unhandled ChannelEdgeInfo type: %T", e1)
	}
}

func assertEdgeInfo2Equal(t *testing.T, e1 *ChannelEdgeInfo2,
	e2 *ChannelEdgeInfo2) {

	if e1.Features.IsEmpty() {
		require.True(t, e2.Features.IsEmpty())
	} else {
		require.Equal(t, e1.Features, e2.Features)
	}
	e1.Features = lnwire.RawFeatureVector{}
	e2.Features = lnwire.RawFeatureVector{}

	require.Equal(t, e1, e2)
}

func assertEdgeInfo1Equal(t *testing.T, e1 *ChannelEdgeInfo1,
	e2 *ChannelEdgeInfo1) {
...
func createChannelEdge(db kvdb.Backend, node1, node2 *LightningNode) (
	*ChannelEdgeInfo1, *ChannelEdgePolicyWithNode,
	*ChannelEdgePolicyWithNode) {
	...
	edge1 := &ChannelEdgePolicyWithNode{
	...
	edge2 := &ChannelEdgePolicyWithNode{
		ChannelEdgePolicy1: ChannelEdgePolicy1{
			SigBytes:                  testSig.Serialize(),
			ChannelID:                 chanID,
...
func assertEdgeWithNoPoliciesInCache(t *testing.T, g *ChannelGraph,
	...
	var (
		node1Bytes = e.Node1Bytes()
		node2Bytes = e.Node2Bytes()
	)

	// Let's check the internal view first.
	require.NotEmpty(t, g.graphCache.nodeChannels[node1Bytes])
	require.NotEmpty(t, g.graphCache.nodeChannels[node2Bytes])
	...
	require.Contains(
		t, g.graphCache.nodeChannels[node1Bytes], e.GetChanID(),
	)
	require.Equal(
		t, expectedNode1Channel,
		g.graphCache.nodeChannels[node1Bytes][e.GetChanID()],
	)
	...
	require.Contains(
		t, g.graphCache.nodeChannels[node2Bytes], e.GetChanID(),
	)
	require.Equal(
		t, expectedNode2Channel,
		g.graphCache.nodeChannels[node2Bytes][e.GetChanID()],
	)

	// The external view should reflect this as well.
	var foundChannel *DirectedChannel
	err := g.graphCache.ForEachChannel(
		node1Bytes, func(c *DirectedChannel) error {
			if c.ChannelID == e.GetChanID() {
				foundChannel = c
			}
	...
	err = g.graphCache.ForEachChannel(
		node2Bytes, func(c *DirectedChannel) error {
			if c.ChannelID == e.GetChanID() {
				foundChannel = c
			}
...
func assertEdgeWithPolicyInCache(t *testing.T, g *ChannelGraph,
	e models.ChannelEdgeInfo, p *ChannelEdgePolicyWithNode, policy1 bool) {
	...
	c1, ok := g.graphCache.nodeChannels[node1Bytes][chanID]
	require.True(t, ok)
	...
	c2, ok := g.graphCache.nodeChannels[node2Bytes][chanID]
	require.True(t, ok)
...
func randEdgePolicy(chanID uint64, db kvdb.Backend) *ChannelEdgePolicyWithNode {
	update := prand.Int63()
...
func newEdgePolicy(chanID uint64, db kvdb.Backend,
	updateTime int64) *ChannelEdgePolicyWithNode {

	return &ChannelEdgePolicyWithNode{
		ChannelEdgePolicy1: ChannelEdgePolicy1{
			ChannelID:                 chanID,
			LastUpdate:                time.Unix(updateTime, 0),
			MessageFlags:              1,
...
func TestGraphTraversal(t *testing.T) {
	...
	err = graph.ForEachChannel(func(ei models.ChannelEdgeInfo,
		...
		_ *ChannelEdgePolicyWithNode) error {
	...
	err = firstNode.ForEachChannel(graph.DB(), nil,
		func(_ kvdb.Backend, _ kvdb.RTx, _ models.ChannelEdgeInfo,
			outEdge, inEdge *ChannelEdgePolicyWithNode) error {
...
func assertNumChans(t *testing.T, graph *ChannelGraph, n int) {
	...
	if err := graph.ForEachChannel(func(models.ChannelEdgeInfo,
		*ChannelEdgePolicyWithNode, *ChannelEdgePolicyWithNode) error {
	...
	checkPolicies := func(node *LightningNode, expectedIn, expectedOut bool) {
		...
		err := node.ForEachChannel(graph.DB(), nil,
			func(_ kvdb.Backend, _ kvdb.RTx,
				_ models.ChannelEdgeInfo, outEdge,
				inEdge *ChannelEdgePolicyWithNode) error {
...
func compareEdgePolicies(a, b *ChannelEdgePolicyWithNode) error {
...
func TestBatchedUpdateEdgePolicy(t *testing.T) {
	...
	updates := []*ChannelEdgePolicyWithNode{edge1, edge2}
	...
	for _, update := range updates {
		wg.Add(1)
		go func(update *ChannelEdgePolicyWithNode) {
			defer wg.Done()
			...
			for _, n := range nodes {
				err := n.ForEachChannel(
					graph.db, tx, func(_ kvdb.Backend,
						...
						policy2 *ChannelEdgePolicyWithNode) error {
