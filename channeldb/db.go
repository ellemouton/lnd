...
// NodeAddrs consults the channel database for all addresses known to the passed
// node public key.
func (d *DB) NodeAddrs(nodePub *btcec.PublicKey) ([]net.Addr, error) {
	linkNode, err := d.channelStateDB.linkNodeDB.FetchLinkNode(nodePub)
	if err != nil {
		return nil, err
	}

	return linkNode.Addresses, nil
}
