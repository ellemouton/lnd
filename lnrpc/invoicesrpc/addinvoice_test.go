...
func TestBuildBlindedPathWithDummyHops(t *testing.T) {
	...
	// Check that all Alice's info is correct. Alice should have just
	// the next dummy hop's private key.
	hop = path.Hops[2]
	data, blindingPoint = decryptAndDecodeHopData(
		t, privA, blindingPoint, hop.CipherText,
	)
	...
	require.True(t, data.Constraints.IsNone())
	require.True(t, data.NextDummyPrivKey.IsSome())

	// Extract the dummy private key and use it to unwrap the next payload.
	dummyPriv, _ := btcec.PrivKeyFromBytes(
		data.NextDummyPrivKey.UnwrapOrFail(t).Val,
	)

	// This next payload is yet another dummy hop.
	hop = path.Hops[3]
	data, blindingPoint = decryptAndDecodeHopData(
		t, dummyPriv, blindingPoint, hop.CipherText,
	)
	require.True(t, data.ShortChannelID.IsNone())
	require.True(t, data.RelayInfo.IsSome())
	require.True(t, data.Constraints.IsNone())
