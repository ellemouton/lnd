...
// hop hint the the payment request.
type hopHint struct {
...
func (p policies) addPolicy(channel *channeldb.OpenChannel,
	...
	modifiedHopHint := bestHopHint
	modifiedHopHint.policy.TimeLockDelta = maxTimelock

	p[route.NewVertex(channel.IdentityPub)] = modifiedHopHint

	// Determine if the new policy's channel has a remote bandwidth that is
	// larger that then old policy's channel and return the difference if
	// there is an increase.
	oldPolicyBandwidth := oldHint.channel.LocalCommitment.RemoteBalance
	newPolicyBandwidth := modifiedHopHint.channel.LocalCommitment.RemoteBalance
