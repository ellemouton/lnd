...
func TestGraphClient(t *testing.T) {
	...
	t.Run("FetchNodeFeatures", func(t *testing.T) {
		...
		kit.lnConn.On("GetNodeInfo", mock.Anything, &lnrpc.NodeInfoRequest{
			PubKey:          node1Hex,
			IncludeChannels: false,
		}, mock.Anything).Return(&lnrpc.NodeInfo{
			Node: &lnrpc.LightningNode{
				PubKey: node1Hex,
				Features: map[uint32]*lnrpc.Feature{
					0: {Name: "feature1"},
	...
	t.Run("FetchChannelEdgesByOutpoint", func(t *testing.T) {
		...
		edge, policy1, policy2, err := kit.client.FetchChannelEdgesByOutpoint(
			ctx, &testChanPoint2,
		)
		require.NoError(t, err)
	...
	t.Run("AddrsForNode", func(t *testing.T) {
		...
		kit.lnConn.On("GetNodeInfo", mock.Anything, &lnrpc.NodeInfoRequest{
			PubKey:          node3Hex,
			IncludeChannels: false,
		}, mock.Anything).Return(nil, fmt.Errorf("other error"))

		found, addrs, err = kit.client.AddrsForNode(ctx, node3Pub)
		require.Error(t, err)
		require.False(t, found)
	})
