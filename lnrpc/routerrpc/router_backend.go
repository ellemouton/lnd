...
func marshallWireError(msg lnwire.FailureMessage,
	...
	case *lnwire.FailExpiryTooSoon:
		...
		response.ChannelUpdate, err = marshallChannelUpdate(onionErr.Update)
		if err != nil {
			return err
		}
...
func marshallChannelUpdate(msg lnwire.ChannelUpdate) (*lnrpc.ChannelUpdate,
	error) {
	...
	case *lnwire.ChannelUpdate1:
		return &lnrpc.ChannelUpdate{
			GossipVersion:   lnrpc.GossipVersion_V1,
	...
	case *lnwire.ChannelUpdate2:
		return &lnrpc.ChannelUpdate{
			GossipVersion:    lnrpc.GossipVersion_V2,
			Signature:        update.Signature.Val.ToSignatureBytes(),
			ChainHash:        update.ChainHash.Val[:],
			ChanId:           update.ShortChannelID.Val.ToUint64(),
			DisableFlags:     uint32(update.DisabledFlags.Val),
			TimeLockDelta:    uint32(update.CLTVExpiryDelta.Val),
			HtlcMinimumMsat:  uint64(update.HTLCMinimumMsat.Val),
			HtlcMaximumMsat:  uint64(update.HTLCMaximumMsat.Val),
			BaseFee:          update.FeeBaseMsat.Val,
			FeeRate:          update.FeeProportionalMillionths.Val,
			BlockHeight:      update.BlockHeight.Val,
			ExtraSignedTypes: update.ExtraSignedFields,
		}, nil

	default:
		return nil, fmt.Errorf("unknown channel update type %T", msg)
	}
}
