...
func (d *AuthenticatedGossiper) handleChanUpdate(nMsg *networkMsg,
	...
	// TODO(elle): abstract this check
	switch u := upd.(type) {
	case *lnwire.ChannelUpdate1:
		timestamp := time.Unix(int64(u.Timestamp), 0)
	...
	case *lnwire.ChannelUpdate2:
		if u.BlockHeight.Val-d.latestHeight() > uint32(graph.DefaultChannelPruneExpiry.Hours()/6) {
			log.Errorf("Skewed blockheight (%v) for edge policy of "+
				"short_chan_id(%v), blockheight too far in the future: "+
				"peer=%v, msg=%s, is_remote=%v", u.BlockHeight,
				shortChanID, nMsg.peer, nMsg.msg.MsgType(),
				nMsg.isRemote,
			)
...
func (d *AuthenticatedGossiper) handleAnnSig(nMsg *networkMsg,
	...
	if err := chanAnn.Validate(d.fetchPKScript); err != nil {
