...
func (d *deDupedAnnouncements) addMsg(message networkMsg) {
	...
	case lnwire.ChannelUpdate:
		...
		if ok {
			// If we already have seen this message, compare their ages.
			oldUpdate, ok := mws.msg.(lnwire.ChannelUpdate)
			if !ok {
				log.Errorf("Expected lnwire.ChannelUpdate, "+
					"got: %T", mws.msg)
			...
			if err != nil {
				log.Errorf("Unable to compare channel update ages: "+
					"new=%T old=%T err=%v", msg, oldUpdate, err)
				return
			}

			// If we already had a newer message, ignore this one.
			...
			// If this is newer, replace the existing message and reset
			// the sender set.
		...
		// If this is the first sighting or same-age duplicate, keep it and
		// merge senders.
		if !ok {
			mws = msgWithSenders{
				msg:     msg,
	...
	case lnwire.NodeAnnouncement:
		...
		if ok {
			...
			if !ok {
				log.Errorf("Expected lnwire.NodeAnnouncement, "+
					"got: %T", mws.msg)
				return
			}
			...
			if err != nil {
				log.Errorf("Unable to compare node announcement "+
					"ages: new=%T old=%T err=%v", msg, oldAnn, err)
				return
			}
		...
		// Add to senders map if it's the same as we had, or create a new
		// entry if this is the first sighting.
		if !ok {
			mws = msgWithSenders{
				msg:     msg,
				isLocal: !message.isRemote,
...
func (d *AuthenticatedGossiper) processZombieUpdate(_ context.Context,
	...
	// come through again.
	err = d.cfg.Graph.MarkEdgeLive(msg.GossipVersion(), scid)
	switch {
...
func (d *AuthenticatedGossiper) isMsgStale(_ context.Context,
	...
	case lnwire.ChannelUpdate:
		...
		_, p1, p2, err := d.cfg.Graph.GetChannelByID(
			msg1.GossipVersion(), msg1.ShortChannelID,
		)
...
func (d *AuthenticatedGossiper) handleNodeAnnouncement(ctx context.Context,
	...
	log.Debugf("Processing NodeAnnouncement: peer=%v, %v, "+
		"node=%x, source=%x", nMsg.peer, nodeAnn.TimestampDesc(), nodePub,
	...
	// We'll quickly ask the router if it already has a newer update for this
	// node so we can skip validating signatures if not required.
	if d.cfg.Graph.IsStaleNode(
		ctx, nodeAnn.GossipVersion(), nodePub, nodeAnn.UpdateTimestamp(),
	) {
	...
	log.Debugf("Processed NodeAnnouncement: peer=%v, %v, "+
		"node=%x, source=%x", nMsg.peer, nodeAnn.TimestampDesc(), nodePub,
		nMsg.source.SerializeCompressed())
...
func (d *AuthenticatedGossiper) handleChanAnnouncement(ctx context.Context,
	...
	if d.cfg.Graph.IsKnownEdge(ann.GossipVersion(), scid) {
		nMsg.err <- nil
		return nil, true
	}
...
func (d *AuthenticatedGossiper) validateFundingTransaction(_ context.Context,
	...
	if err != nil {
		...
		case strings.Contains(err.Error(), "out of range"):
			...
			zErr := d.cfg.Graph.MarkZombieEdge(
	...
	// Recreate the expected funding output script from the edge data to ensure
	// that declared channel data corresponds to reality on chain.
	var fundingPkScript []byte
	switch ann := ann.(type) {
	case *lnwire.ChannelAnnouncement1:
		// TODO(elle): remove this v1 reconstruction once the funding
		// manager emits native ChannelAnnouncement2 for taproot channels.
