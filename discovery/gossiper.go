...
// wishes to update a particular set of channels. New ChannelUpdate1 messages
...
func (d *AuthenticatedGossiper) ProcessRemoteAnnouncement(msg lnwire.Message,
	...
	case lnwire.ChannelAnnouncement:
		...
		ownErr := fmt.Errorf("ignoring remote %s for own channel",
			m.MsgType())

		var (
			node1ID = m.Node1KeyBytes()
			node2ID = m.Node2KeyBytes()
		)

		if bytes.Equal(node1ID[:], ownKey) ||
			bytes.Equal(node2ID[:], ownKey) {
...
// channelUpdateID is a unique identifier for ChannelUpdate1 messages, as
...
func (d *AuthenticatedGossiper) networkHandler() {
	...
	for {
		...
		// ChannelUpdate1 for the set of affected clients.
...
func (d *AuthenticatedGossiper) processChanPolicyUpdate(
	...
	for _, edgeInfo := range edgesToUpdate {
		...
		// retrieve our ChannelUpdate1 to broadcast.
...
func (d *AuthenticatedGossiper) processZombieUpdate(
	...
	// come through again.
	baseScid := lnwire.NewShortChanIDFromInt(chanInfo.GetChanID())
...
func IsKeepAliveUpdate(update lnwire.ChannelUpdate,
	...
	case *lnwire.ChannelUpdate2:
		prev, ok := prevPolicy.(*channeldb.ChannelEdgePolicy2)
		if !ok {
		...
		if upd.GetBaseFee() != prev.BaseFee() {
			return false, nil
		}
		if upd.GetFeeRate() != prev.FeeRate() {
			return false, nil
		}
		if upd.GetTimeLock() != prev.CLTVExpiryDelta {
			return false, nil
		}
		if upd.HTLCMinimumMsat != prev.HTLCMinimumMsat {
			return false, nil
		}
		if upd.HTLCMaximumMsat != prev.HTLCMaximumMsat {
			return false, nil
		}
		if !bytes.Equal(upd.ExtraOpaqueData, prev.ExtraOpaqueData) {
			return false, nil
...
func (d *AuthenticatedGossiper) handleChanAnnouncement(nMsg *networkMsg,
	...
	// Launch a new goroutine to handle each ChannelUpdate1, this is to
	...
	for _, cu := range channelUpdates {
		...
		// Mark the ChannelUpdate1 as processed. This ensures that a
		// subsequent announcement in the option-scid-alias case does
		// not re-use an old ChannelUpdate1.
		cu.processed = true
		...
		go func(updMsg *networkMsg) {
			...
			// ChannelUpdate1 to be in this cache.
			default:
...
func (d *AuthenticatedGossiper) updateWithinRebroadcastInterval(
	...
	case *lnwire.ChannelUpdate1:
		pol, ok := policy.(*channeldb.ChannelEdgePolicy1)
		if !ok {
			return false, fmt.Errorf("expected chan edge policy 1")
		}
	...
	case *lnwire.ChannelUpdate2:
		pol, ok := policy.(*channeldb.ChannelEdgePolicy2)
		if !ok {
			return false, fmt.Errorf("expected chan edge policy 2")
		}
...
func (d *AuthenticatedGossiper) handleChanUpdate(nMsg *networkMsg,
	...
	case channeldb.ErrEdgeNotFound:
		...
		// ChannelUpdate1 is later reprocessed.
		return nil, false
	...
	// ShortChannelID in the ChannelUpdate1 to avoid the router having to
...
func buildChanProof(ann lnwire.ChannelAnnouncement) (
	*channeldb.ChannelAuthProof1, error) {
	...
	case *lnwire.ChannelAnnouncement1:
		return &channeldb.ChannelAuthProof1{
