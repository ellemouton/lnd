...
// pub key if it is less than our current accumulator value. It does so by
// comparing the current hash accumulator value with the passed byte slice.
// When comparing, we skip the first byte as it's 50/50.
//
// NOTE: this is part of the hashAccumulator interface.
...
func (*noOpHashAccumulator) skipNode(pubKey route.Vertex) bool {
	return false
}
