...
type mockGraphSource struct {
	...
	edges         map[uint64][]channeldb.ChannelEdgePolicyWithNode
...
func newMockRouter(height uint32) *mockGraphSource {
	return &mockGraphSource{
		...
		edges:         make(map[uint64][]channeldb.ChannelEdgePolicyWithNode),
...
func (r *mockGraphSource) UpdateEdge(edge *channeldb.ChannelEdgePolicyWithNode,
	_ ...batch.SchedulerOption) error {
	...
	if len(r.edges[edge.ChannelID]) == 0 {
		r.edges[edge.ChannelID] = make(
			[]channeldb.ChannelEdgePolicyWithNode, 2,
		)
	}
...
func (r *mockGraphSource) ForAllOutgoingChannels(cb func(tx kvdb.RTx,
	...
	c *channeldb.ChannelEdgePolicyWithNode) error) error {
...
func (r *mockGraphSource) ForEachChannel(_ func(chanInfo models.ChannelEdgeInfo,
...
func (r *mockGraphSource) GetChannelByID(chanID lnwire.ShortChannelID) (
	models.ChannelEdgeInfo, *channeldb.ChannelEdgePolicyWithNode,
	*channeldb.ChannelEdgePolicyWithNode, error) {
	...
	var edge1 *channeldb.ChannelEdgePolicyWithNode
	if !reflect.DeepEqual(edges[0], channeldb.ChannelEdgePolicyWithNode{}) {
		edge1 = &edges[0]
	}

	var edge2 *channeldb.ChannelEdgePolicyWithNode
	if !reflect.DeepEqual(edges[1], channeldb.ChannelEdgePolicyWithNode{}) {
		edge2 = &edges[1]
	}
...
func TestSignatureAnnouncementLocalFirst(t *testing.T) {
	...
	// The local ChannelUpdate1 should now be sent directly to the remote peer,
...
func TestReceiveRemoteChannelUpdateFirst(t *testing.T) {
	...
	// Recreate the case where the remote node is sending us its ChannelUpdate1
