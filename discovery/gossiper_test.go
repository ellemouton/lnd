...
func (r *mockGraphSource) IsStaleEdgePolicy(
	...
	if !ok {
		...
		// it is stale or not.
		return time.Since(policy.LastUpdate) > graph.DefaultChannelPruneExpiry
	}
	...
	case lnwire.GossipVersion2:
		...
		case policy.IsNode1() &&
			...
			return edges[0].LastBlockHeight >= policy.LastBlockHeight

		case !policy.IsNode1() &&
			!reflect.DeepEqual(edges[1], models.ChannelEdgePolicy{}):

			return edges[1].LastBlockHeight >= policy.LastBlockHeight
	...
	default:
		...
		case policy.IsNode1() &&
			...
			return !policy.LastUpdate.After(edges[0].LastUpdate)

		case !policy.IsNode1() &&
			!reflect.DeepEqual(edges[1], models.ChannelEdgePolicy{}):

			return !policy.LastUpdate.After(edges[1].LastUpdate)
...
func createTestCtx(t *testing.T, startHeight uint32, isChanPeer bool) (
	*testCtx, error) {
	...
	signAliasUpdate := func(lnwire.ChannelUpdate) error { return nil }
