...
// BlockingCGOpt is used to create a cancellable context that will NOT be
...
// CustomTimeoutCGOpt is used to create a cancellable context with a custom
...
func CustomTimeoutCGOpt(timeout time.Duration) ContextGuardOption {
	return func(o *ctxGuardOptions) {
		o.withTimeout = true
		o.timeout = timeout
	}
...
// TimeoutCGOpt is used to create a cancellable context with a default timeout.
...
func TimeoutCGOpt() ContextGuardOption {
	return func(o *ctxGuardOptions) {
		o.withTimeout = true
...
func (g *ContextGuard) WithCtx(ctx context.Context,
	...
	if opts.blocking {
		if opts.withTimeout {
			return g.withCtxBlockingTimeout(ctx, opts.timeout)
		}

		return g.withCtxBlocking(ctx)
	}
	...
	if opts.withTimeout {
		return g.withCtxQuitTimeout(ctx, opts.timeout)
	}

	return g.withCtxQuit(ctx)
}

// withCtxBlockingTimeout is used to derive a cancellable context with a timeout
// that will NOT be cancelled if the main quit signal is triggered, to block
// shutdown of important tasks. The context will be cancelled if the timeout is
// reached.
func (g *ContextGuard) withCtxBlockingTimeout(ctx context.Context,
...
func (g *ContextGuard) withCtxBlocking(ctx context.Context) (context.Context,
	func()) {
	ctx, cancel := context.WithCancel(ctx)
...
// withCtxQuitTimeout is used to derive a cancellable context that will be cancelled
// if the main quit signal is triggered, the parent context is cancelled or the
// timeout is reached.
//
// NOTE: the caller must hold the ContextGuard's mutex before calling this
// function.
func (g *ContextGuard) withCtxQuitTimeout(ctx context.Context,
	timeout time.Duration) (context.Context, context.CancelFunc) {

	ctx, cancel := context.WithTimeout(ctx, timeout)

	cancel = g.ctxQuit(ctx, cancel)

	return ctx, cancel
}

// withCtxQuit is used to derive a cancellable context that will be cancelled if
// the main quit signal is triggered or the parent context is cancelled.
//
// NOTE: the caller must hold the ContextGuard's mutex before calling this
// function.
func (g *ContextGuard) withCtxQuit(ctx context.Context) (context.Context,
	context.CancelFunc) {
...
// ctxQuit spins off a goroutine that will block until the passed context
...
// NOTE: the caller must hold the ContextGuard's mutex before calling this
// function.
func (g *ContextGuard) ctxQuit(ctx context.Context,
	cancel context.CancelFunc) context.CancelFunc {

	cancel = g.addCancelFn(cancel)
...
// addCancelFn adds a context cancel function to the manager and returns a
// call-back which can safely be used to cancel the context.
//
// NOTE: the caller must hold the ContextGuard's mutex before calling this
// function.
func (g *ContextGuard) addCancelFn(
	cancel context.CancelFunc) context.CancelFunc {
...
// cancelCtxFn returns a call-back that can be used to cancels the context
