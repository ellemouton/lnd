...
func (f *Manager) start() error {
	nonces, err := f.cfg.ChannelDB.GetAllAnnouncementNonces()
	if err != nil {
		return err
	}

	for chanID, n := range nonces {
		f.annSigNonces[chanID] = annSigNonces{
			received: &receivedNonces{
				btc:  n.Btc,
				node: n.Node,
			},
...
func (f *Manager) stateStep(channel *channeldb.OpenChannel,
	...
	case addedToRouterGraph:
		...
		f.nonceMtx.Lock()
		delete(f.annSigNonces, chanID)
		f.nonceMtx.Unlock()

		err = f.cfg.ChannelDB.DeleteAnnouncementNonces(chanID)
		if err != nil {
...
func (f *Manager) sendChannelReady(completeChan *channeldb.OpenChannel,
	...
	if completeChan.ChanType.IsTaproot() {
		...
		announceNonces, ok := f.annSigNonces[chanID]
		if public && (!ok || announceNonces.sent == nil) {
			btcNonce, nodeNonce, err := f.genAnnouncementNonces(
				completeChan,
			)
			if err != nil {
				f.nonceMtx.Unlock()
			...
			announceNonces = f.annSigNonces[chanID]
		}
		f.nonceMtx.Unlock()
		...
		if public {
			nodeNonce := lnwire.Musig2Nonce(announceNonces.sent.node.PubNonce)
			channelReadyMsg.AnnouncementNodeNonce = fn.Some(nodeNonce)

			btcNonce := lnwire.Musig2Nonce(announceNonces.sent.btc.PubNonce)
			channelReadyMsg.AnnouncementBitcoinNonce = fn.Some(btcNonce)
		}
	}
...
func (f *Manager) receivedChannelReady(node *btcec.PublicKey,
	...
	if channel.ChanType.IsTaproot() && isPublic {
		nonces, err := f.cfg.ChannelDB.GetAnnouncementNonces(chanID)
		...
		f.nonceMtx.Lock()
		aggNonces, ok := f.annSigNonces[chanID]
		if !ok {
			f.annSigNonces[chanID] = annSigNonces{
				received: &receivedNonces{
					node: nonces.Node,
			...
			aggNonces.received = &receivedNonces{
				node: nonces.Node,
				btc:  nonces.Btc,
			}
		}
...
func (f *Manager) extractAnnounceParams(c *channeldb.OpenChannel) (
	...
	// we'll use this value within our ChannelUpdate1. This constraint is
	...
	// we'll use this value within our ChannelUpdate1. This value must be <=
...
// addToRouterGraph sends a ChannelAnnouncement1 and a ChannelUpdate1 to the
...
// ChannelUpdate1 they understand. ourPolicy may be set for various
...
func (f *Manager) addToRouterGraph(completeChan *channeldb.OpenChannel,
	...
	ann, err := f.newChanAnnouncement(
		...
		completeChan.ChanType, completeChan, false,
	)
	if err != nil {
	...
	// Send ChannelAnnouncement1 and ChannelUpdate1 to the gossiper to add
...
func (f *Manager) annAfterSixConfs(completeChan *channeldb.OpenChannel,
	...
	// we'll only send our NodeAnnouncement1 to our counterparty to ensure we
...
func (f *Manager) genAnnouncementNonces(channel *channeldb.OpenChannel) (
	*musig2.Nonces, *musig2.Nonces, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("unable to generate musig "+
...
func (f *Manager) handleChannelReady(peer lnpeer.Peer, //nolint:funlen
	...
	// provide a ChannelUpdate1 to the remote peer. This is done before the
	...
	if channel.NegotiatedAliasFeature() {
		...
		if len(aliases) == 0 {
			...
			if public && annSendNonces != nil {
				nodeNonce := lnwire.Musig2Nonce(annSendNonces.node.PubNonce)
				channelReadyMsg.AnnouncementNodeNonce = fn.Some(nodeNonce)

				btcNonce := lnwire.Musig2Nonce(annSendNonces.btc.PubNonce)
				channelReadyMsg.AnnouncementBitcoinNonce = fn.Some(btcNonce)
			}
	...
	if channel.ChanType.IsTaproot() {
		...
		announcementSendNonces, ok := f.annSigNonces[chanID]
		if public && !ok {
			f.annSigNonces[chanID] = annSigNonces{
				sent: annSendNonces,
			}
			announcementSendNonces = f.annSigNonces[chanID]
		}
		...
		if public {
			...
			var received receivedNonces
			msg.AnnouncementNodeNonce.WhenSome(
				func(nonce lnwire.Musig2Nonce) {
					received.node = nonce
				},
			)

			msg.AnnouncementBitcoinNonce.WhenSome(
				func(nonce lnwire.Musig2Nonce) {
					received.btc = nonce
				},
			)

			announcementSendNonces.received = &received

			f.annSigNonces[chanID] = announcementSendNonces
		}
		f.nonceMtx.Unlock()
	...
	var (
		btcNonce  *lnwire.Musig2Nonce
		nodeNonce *lnwire.Musig2Nonce
	)
	msg.AnnouncementBitcoinNonce.WhenSome(func(nonce lnwire.Musig2Nonce) {
		btcNonce = &nonce
	})
	msg.AnnouncementNodeNonce.WhenSome(func(nonce lnwire.Musig2Nonce) {
		nodeNonce = &nonce
	})

	if btcNonce != nil && nodeNonce != nil {
		err = f.cfg.ChannelDB.SaveAnnouncementNonces(
		...
		if err != nil {
			log.Errorf("unable to insert announcement nonces: %v",
				err)
...
func (f *Manager) handleChannelReadyReceived(channel *channeldb.OpenChannel,
	...
	if channel.IsZeroConf() {
		...
		// ChannelUpdate1 for them.  If an alias does not yet exist,
...
func (f *Manager) newChanAnnouncement(localPubKey,
	...
	chanType channeldb.ChannelType, channel *channeldb.OpenChannel,
	...
	// these values can later be changed by crafting a new ChannelUpdate1.
...
func (f *Manager) newTaprootChanAnnouncement(localPubKey,
	...
	remoteFundingKey *btcec.PublicKey, shortChanID lnwire.ShortChannelID,
	chanID lnwire.ChannelID, fwdMinHTLC, fwdMaxHTLC lnwire.MilliSatoshi,
	ourEdgePolicy models.ChannelEdgePolicy, channel *channeldb.OpenChannel,
	withProof bool) (*chanAnnouncement, error) {

	var ourPolicy *models.ChannelEdgePolicy2
	if ourEdgePolicy != nil {
		var ok bool
		ourPolicy, ok = ourEdgePolicy.(*models.ChannelEdgePolicy2)
		if !ok {
			return nil, fmt.Errorf("expected "+
				"ChannelEdgePolicy1, got: %T", ourEdgePolicy)
		}
	...
	if bytes.Compare(selfBytes, remoteBytes) == -1 {
		...
		var btc1 [33]byte
		copy(btc1[:], localFundingKey.PubKey.SerializeCompressed())
		chanAnn.BitcoinKey1 = fn.Some(btc1)

		var btc2 [33]byte
		copy(btc2[:], remoteFundingKey.SerializeCompressed())
		chanAnn.BitcoinKey2 = fn.Some(btc2)
		...
		var btc1 [33]byte
		copy(btc1[:], remoteFundingKey.SerializeCompressed())
		chanAnn.BitcoinKey1 = fn.Some(btc1)

		var btc2 [33]byte
		copy(btc2[:], localFundingKey.PubKey.SerializeCompressed())
		chanAnn.BitcoinKey2 = fn.Some(btc2)
	...
	chanUpdateAnn := &lnwire.ChannelUpdate2{
		ChainHash:      chainHash,
		ShortChannelID: shortChanID,
		BlockHeight:    shortChanID.BlockHeight,
	...
	case ourPolicy != nil:
		...
		chanUpdateAnn.FeeBaseMsat = ourPolicy.FeeBaseMsat
	...
	chanAnnMsg, err := chanAnn.DigestToSign()
	if err != nil {
		return nil, err
	}
	...
	f.nonceMtx.Lock()
	announcementNonces, ok := f.annSigNonces[chanID]
	if !ok || announcementNonces.received == nil {
		f.nonceMtx.Unlock()

		return nil, fmt.Errorf("havent received all the necessary " +
			"nonces yet")
	}
	if ok && announcementNonces.sent == nil {
		node, btc, err := f.genAnnouncementNonces(channel)
		if err != nil {
			f.nonceMtx.Unlock()

			return nil, fmt.Errorf("error generating nonces")
		}
		announcementNonces.sent = &sentNonces{
			btc:  btc,
			node: node,
		}
	}
...
func (f *Manager) announceChannel(localIDKey, remoteIDKey *btcec.PublicKey,
	...
	// the ChannelUpdate1 announcement messages. The channel proof and node
	// announcements are broadcast to the greater network.
	errChan := f.cfg.SendAnnouncement(ann.chanProof, ann.opts...)
	select {
	case err := <-errChan:
		if err != nil {
			if routing.IsError(err, routing.ErrOutdated,
