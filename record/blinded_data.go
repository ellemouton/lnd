...
type BlindedRouteData struct {
	...
	// path they created was used for the intended purpose. For blinded
	// path payments this can, for example, be set to the pre-image of the
	// payment.
...
// hop in a blinded route. Only the path_id field is set for the final hop.
func NewFinalHopBlindedRouteData(pathID []byte) *BlindedRouteData {
...
func (b *BlindedRouteData) PadBy(n uint64) error {
	// If no padding is needed, then there is nothing to do.
	if n == 0 {
		return nil
	}

	if b.Padding.IsSome() {
		return fmt.Errorf("padding has already been added to the " +
			"blinded route data")
	}
...
// Record creates a tlv.Record that encodes the payment relay (type 10) type
// for an encrypted blob payload.
