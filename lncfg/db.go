...
func (db *DB) GetBackends(ctx context.Context, chanDBPath,
	...
	case EtcdBackend:
		...
		if err != nil {
			return nil, fmt.Errorf("error opening etcd channel DB: %w", err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("error opening etcd graph DB: %w", err)
		}
	...
	case PostgresBackend:
		...
		if err != nil {
			return nil, fmt.Errorf("error opening postgres channel "+
				"DB: %v", err)
		}
