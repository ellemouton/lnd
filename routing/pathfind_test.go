...
func TestBlindedRouteConstruction(t *testing.T) {
	...
	// accordingly.
	blindedEdges, err := blindedPathSet.toRouteHints()
	require.NoError(t, err)
...
func TestLastHopPayloadSize(t *testing.T) {
	...
	oneHopBlindedPayment, err := NewBlindedPaymentPathSet(
		[]*BlindedPayment{{BlindedPath: &sphinx.BlindedPath{
			BlindedHops: []*sphinx.BlindedHopInfo{
				{
	...
	testCases := []struct {
		...
		},
		{
			...
			restrictions: &RestrictParams{
				BlindedPathPaymentSet: oneHopBlindedPayment,
			},
			amount:         amtToForward,
			finalHopExpiry: finalHopExpiry,
		},
		{
			...
			restrictions: &RestrictParams{
				BlindedPathPaymentSet: twoHopBlindedPayment,
			},
			amount:         amtToForward,
			finalHopExpiry: finalHopExpiry,
		},
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.restrictions.BlindedPathPaymentSet != nil {
				paymentPath := tc.restrictions.
					BlindedPathPaymentSet.
					LargestLastHopPayloadPath()
				blindedPath := paymentPath.BlindedPath.
					BlindedHops
				blindedPoint := paymentPath.BlindedPath.
					BlindingPoint
...
func TestFindBlindedPaths(t *testing.T) {
	...
	testChannels := []*testChannel{
		symmetricTestChannel(
			"dave", "alice", 100000, &testChannelPolicy{
				Expiry:  144,
				FeeRate: 400,
				MinHTLC: 1,
				MaxHTLC: 100000000,
		...
		symmetricTestChannel(
			"dave", "bob", 100000, &testChannelPolicy{
				Expiry:   144,
				FeeRate:  400,
				MinHTLC:  1,
				MaxHTLC:  100000000,
		...
		symmetricTestChannel(
			"dave", "charlie", 100000, &testChannelPolicy{
				Expiry:   144,
				FeeRate:  400,
				MinHTLC:  1,
				MaxHTLC:  100000000,
		...
		symmetricTestChannel(
			"bob", "frank", 100000, &testChannelPolicy{
				Expiry:   144,
				FeeRate:  400,
				MinHTLC:  1,
				MaxHTLC:  100000000,
		...
		symmetricTestChannel(
			"eve", "charlie", 100000, &testChannelPolicy{
				Expiry:   144,
				FeeRate:  400,
				MinHTLC:  1,
				MaxHTLC:  100000000,
		...
		symmetricTestChannel(
			"dave", "george", 100000, &testChannelPolicy{
				Expiry:   144,
				FeeRate:  400,
				MinHTLC:  1,
				MaxHTLC:  100000000,
	...
	// blinding feature bit. The G->B path is not chosen since
