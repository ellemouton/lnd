...
func testBasicGraphPathFindingCase(t *testing.T, graphInstance *testGraphInstance,
	...
	for i := 0; i < len(expectedHops)-1; i++ {
		...
		require.Equal(
			t, payload.FwdInfo.NextHop.ToUint64(),
			route.Hops[i+1].ChannelID,
		)
	}
	...
	require.EqualValues(t, payload.FwdInfo.NextHop.ToUint64(), 0)
...
func runNewRoutePathTooLong(t *testing.T, useCache bool) {
	...
	node20 := ctx.keyFromAlias("node-25")
	...
	node21 := ctx.keyFromAlias("node-26")
