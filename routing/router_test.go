...
func (c *testCtx) RestartRouter(t *testing.T) {
	...
	router, err := New(Config{
		RoutingGraph:       NewNodeAgnosticGraph(c.graph),
		Graph:              c.graph,
		Chain:              c.chain,
...
func createTestCtxFromGraphInstanceAssumeValid(t *testing.T,
	...
	sessionSource := &SessionSource{
		GetRoutingGraph: func() (RoutingGraph, func(), error) {
			graph := NewNodeAgnosticGraph(graphInstance.graph)
	...
	router, err := New(Config{
		RoutingGraph:       NewNodeAgnosticGraph(graphInstance.graph),
...
func TestWakeUpOnStaleBranch(t *testing.T) {
	...
	router, err := New(Config{
		RoutingGraph:       NewNodeAgnosticGraph(ctx.graph),
		Graph:              ctx.graph,
		Chain:              ctx.chain,
