...
func createTestCtxFromGraphInstanceAssumeValid(t *testing.T,
	...
	sourceNode, err := graphInstance.graphReader.SourceNode(t.Context())
	require.NoError(t, err)
...
func TestAddEdgeUnknownVertexes(t *testing.T) {
	...
	exists1, err := ctx.graphReader.HasNode(ctxb, pub1)
	require.NoError(t, err, "unable to query graph")
	require.False(t, exists1)

	exists2, err := ctx.graphReader.HasNode(ctxb, pub2)
	require.NoError(t, err, "unable to query graph")
	...
	copy1, err := ctx.graphReader.FetchNode(ctxb, pub1)
	require.NoError(t, err, "unable to fetch node")

	require.Equal(t, n1.Alias, copy1.Alias)

	copy2, err := ctx.graphReader.FetchNode(ctxb, pub2)
	require.NoError(t, err, "unable to fetch node")
