...
type SessionSource struct {
	// GetGraph returns a Graph that can be used for path finding for a
	// specific payment. The clean-up call call-back must be called once
...
func (m *SessionSource) NewPaymentSession(p *LightningPayment) (
	...
	session, err := newPaymentSession(
		...
		m.GetGraph, m.MissionControl, m.PathFindingConfig,
	)
	if err != nil {
		return nil, err
	}
