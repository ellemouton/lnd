...
// as additional edges because they are append at the end of a normal route.
type BlindedEdge struct {
	policy *models.CachedEdgePolicy

	// blindedPayment is the BlindedPayment that this blinded edge is
	// associated with.
	blindedPayment *BlindedPayment
...
func NewBlindedEdge(policy *models.CachedEdgePolicy, payment *BlindedPayment,
	...
	if hopIndex >= len(payment.BlindedPath.BlindedHops) {
		return nil, fmt.Errorf("the hop index %d is invalid given "+
			"that there are only %d hops in the blinded path",
			hopIndex, len(payment.BlindedPath.BlindedHops))
	}
