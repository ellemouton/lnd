...
// then its peer will switch to the new IP address and reconnect to it.
func testReconnectAfterIPChange(net *lntest.NetworkHarness, t *harnessTest) {
	...
	for _, addr := range advertisedAddrs {
		daveArge = append(daveArge, "--externalip="+addr)
	}

	// Create a new node, Dave, and ensure that his initial P2P port is
	// ip1 derived above.
	dave := net.NewNode(t.t, "Dave", daveArge, withP2PPort(ip1))
	defer shutdownAndAssert(net, t, dave)
	...
	// assert that Dave and Charlie reconnect after Dave changes to his
	// second advertised address.
	assertConnected(t, dave, charlie)
}

// withP2PPort is an optional function used to set the P2P port that a node
// should use.
func withP2PPort(port int) lntest.NodeOption {
