...
func testInvoiceRoutingHints(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Launch notification clients for Alice and Bob nodes, such they can
	// get notified when there are updates in channel policies.
	aliceSub := subscribeGraphNotifications(t, ctxb, net.Alice)
	defer close(aliceSub.quit)
	bobSub := subscribeGraphNotifications(t, ctxb, net.Bob)
	defer close(bobSub.quit)
	...
	require.NotEqual(t.t, 0, aliceBobChan2ID, "channel between alice and bob not found")

	// Ensure that all the initail policy values for both channels are as
	// expected.
	expectedPolicy := &lnrpc.RoutingPolicy{
		FeeBaseMsat:      defaultFeeBase,
		FeeRateMilliMsat: defaultFeeRate,
	...
	for _, graphSub := range graphSubs {
		waitForChannelUpdate(
			...
			[]expectedChanUpdate{
				...
				{net.Bob.PubKeyStr, expectedPolicy,
					chanPointBob2},
			},
		)
	}
