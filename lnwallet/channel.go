...
func (lc *LightningChannel) validateFeeRate(feePerKw chainfee.SatPerKWeight) error {
	...
	// update.
	_, txWeight := lc.availableBalance()
	...
	baseBalance := localBalance + oldFee
	...
	if balanceAfterFee.ToSatoshis() < lc.channelState.LocalChanCfg.ChanReserve {
		return fmt.Errorf("cannot apply fee_update=%v sat/kw, "+
			...
			int64(feePerKw), balanceAfterFee,
			lc.channelState.LocalChanCfg.ChanReserve)
	}
...
func (lc *LightningChannel) MaxFeeRate(maxAllocation float64,
	...
	// The max fee rate should be low enough so that we preserve our local
	// reserve amount if the max fee rate is used.
	localReserve := float64(lc.LocalChanReserve())
	if feeBalance-maxFee < localReserve {
		maxFee = feeBalance - localReserve
	}
