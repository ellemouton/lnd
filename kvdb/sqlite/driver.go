...
func parseArgs(funcName string, args ...interface{}) (context.Context,
	*Config, string, string, string, error) {
	...
	return ctx, config, dbPath, dbName, prefix, nil
}

// createDBDriver is the callback provided during driver registration that
// creates, initializes, and opens a database for use.
func createDBDriver(args ...interface{}) (walletdb.DB, error) {
	ctx, config, filePath, fileName, prefix, err := parseArgs(
		"Create", args...,
	)
	if err != nil {
		return nil, err
	...
	return NewSqliteBackend(ctx, config, filePath, fileName, prefix)
}
