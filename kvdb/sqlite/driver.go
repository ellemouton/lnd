...
func parseArgs(funcName string, args ...interface{}) (context.Context, *Config,
	...
	if len(args) != 4 {
		return nil, nil, "", "", fmt.Errorf("invalid number of "+
			...
			"sql.Config, string", dbType, funcName,
		)
	}
	...
	return ctx, config, fileName, prefix, nil
}

// createDBDriver is the callback provided during driver registration that
// creates, initializes, and opens a database for use.
func createDBDriver(args ...interface{}) (walletdb.DB, error) {
	ctx, config, filename, prefix, err := parseArgs("Create", args...)
	if err != nil {
		return nil, err
	}

	return NewSqliteBackend(ctx, config, filename, prefix)
}

// openDBDriver is the callback provided during driver registration that opens
// an existing database for use.
func openDBDriver(args ...interface{}) (walletdb.DB, error) {
	ctx, config, filename, prefix, err := parseArgs("Open", args...)
	if err != nil {
		return nil, err
	}
