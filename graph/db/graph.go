...
func (c *ChannelGraph) ForEachNodeDirectedChannel(tx kvdb.RTx,
	...
	dbCallback := func(_ kvdb.RTx, e *models.ChannelEdgeInfo, p1,
		...
		channel, err := marshalNodeDirectedChannel(
			node, toNodeFeatures, e, p1, p2,
		)
		// If there is a decoding error, skip this edge.
		if err != nil {
			return nil
		}
...
func marshalNodeDirectedChannel(node route.Vertex,
	toNodeFeatures *lnwire.FeatureVector, e *models.ChannelEdgeInfo,
	p1, p2 *models.ChannelEdgePolicy) (*DirectedChannel, error) {
...
func (c *ChannelGraph) ForEachNodeCached(cb func(node route.Vertex,
	...
	// main function expects.
	return c.forEachNode(func(tx kvdb.RTx,
		node *models.LightningNode) error {

		channels := make(map[uint64]*DirectedChannel)

		toNodeFeatures, err := c.FetchNodeFeatures(node.PubKeyBytes)
		if err != nil {
			return err
		}
		...
		err = c.ForEachNodeChannelTx(tx, node.PubKeyBytes,
			func(tx kvdb.RTx, e *models.ChannelEdgeInfo,
				...
				channel, err := marshalNodeDirectedChannel(
					node.PubKeyBytes, toNodeFeatures, e, p1,
					p2,
				)
				// If there is a decoding error, skip this edge.
				if err != nil {
...
// executed under the same read transaction.
func (c *ChannelGraph) ForEachNode(cb func(tx NodeTx) error) error {
	return c.forEachNode(func(tx kvdb.RTx,
		node *models.LightningNode) error {
...
// chanGraphNodeTx is an implementation of the NodeTx interface backed by the
...
// A compile-time constraint to ensure chanGraphNodeTx implements the NodeTx
// interface.
var _ NodeTx = (*chanGraphNodeTx)(nil)
...
//
// NOTE: This is a part of the NodeTx interface.
func (c *chanGraphNodeTx) Node() *models.LightningNode {
	return c.node
}

// FetchNode fetches the node with the given pub key under the same transaction
// used to fetch the current node. The returned node is also a NodeTx and any
// operations on that NodeTx will also be done under the same transaction.
//
// NOTE: This is a part of the NodeTx interface.
func (c *chanGraphNodeTx) FetchNode(nodePub route.Vertex) (NodeTx, error) {
