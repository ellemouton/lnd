...
func (c *ChannelGraph) ForEachChannel(_ context.Context,
	...
	return c.db.View(func(tx kvdb.RTx) error {
		...
		return kvdb.ForAll(edgeIndex,
			func(k, edgeInfoBytes []byte) error {
				var chanID [8]byte
				copy(chanID[:], k)
...
func (c *ChannelGraph) DisconnectBlockAtHeight(height uint32) (
	...
	if err := kvdb.Update(c.db, func(tx kvdb.RwTx) error {
		...
		for k, v := cursor.Seek(chanIDStart[:]); k != nil &&
			bytes.Compare(k, chanIDEnd[:]) < 0; k, v = cursor.Next() { //nolint:lll
		...
		for k, _ := pruneCursor.Seek(pruneKeyStart[:]); k != nil &&
			bytes.Compare(k, pruneKeyEnd[:]) <= 0; k, _ = pruneCursor.Next() { //nolint:lll
...
// boolean.
func (c *ChannelGraph) HasLightningNode(_ context.Context, nodePub [33]byte) (time.Time, bool,
	error) {
...
func nodeTraversal(tx kvdb.RTx, nodePub []byte, db kvdb.Backend,
	...
	traversal := func(tx kvdb.RTx) error {
		...
		for nodeEdge, _ := edgeCursor.Seek(nodeStart[:]); bytes.HasPrefix(nodeEdge, nodePub); nodeEdge, _ = edgeCursor.Next() { //nolint:lll
			...
			err = cb(
				NewKVDBRTx(tx), &edgeInfo, outgoingPolicy,
				incomingPolicy,
			)
			if err != nil {
				return err
...
// traversal.
func (c *ChannelGraph) ForEachNodeChannel(_ context.Context,
...
// end of the channel.
func (c *ChannelGraph) FetchOtherNode(tx RTx,
	...
	*models.LightningNode, error) {
	...
	return targetNode, fetchNodeFunc(kvdbTx)
}
...
func (c *ChannelGraph) FetchChannelEdgesByOutpoint(_ context.Context,
