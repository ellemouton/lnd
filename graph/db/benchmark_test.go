...
func TestPopulateViaMigration(t *testing.T) {
	...
	// ======= STEP 2 (ignore if source is postgres) ==============
	...
	// ======= STEP 3 (only if source is bbolt!) ============
	...
	// ======= STEP 4 (only if source is sqlite!) ============
	// If your source destination is sqlite, then set this to the name of
	// the sqlte file that contains the channel graph data.
	const sourceSQLiteName = "channel.sqlite"

	// ======= STEP 5 (only if source is postgres!) ============
	...
	// Postgres.
	testSQLite := true // false // if you want to test a postgres destination.

	// ======== STEP 7 (only if destination is sqlite) ========
	...
	// ======== STEP 9 (only if destination is postgres) ========
