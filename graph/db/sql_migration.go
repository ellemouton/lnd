...
func MigrateGraphToSQL(ctx context.Context, kvStore *KVStore,
	...
	err := kvStore.ForEachNode(func(nodeTx NodeRTx) error {
		_, err := insertV1Node(ctx, sqlDB, nodeTx.Node())
		if err != nil {
	...
	migrateChanPolicy := func(dbInfo *dbChanInfo,
		...
		isNode1 := policy.ChannelFlags&lnwire.ChanUpdateDirection == 0
		nodeID := dbInfo.node1ID
		if !isNode1 {
			nodeID = dbInfo.node2ID
		}

		return insertChanPolicy(
			ctx, sqlDB, dbInfo.channelID, nodeID, policy,
		)
	}
...
func migrateSourceNode(ctx context.Context, kvStore *KVStore,
	...
	sourceNodeID, err := sqlDB.GetNodeIDByPubKeyAndVersion(
	...
	err = sqlDB.AddSourceNode(ctx, sourceNodeID)
	if err != nil {
