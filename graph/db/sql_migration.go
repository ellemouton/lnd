...
func migrateNodes(ctx context.Context, kvBackend kvdb.Backend,
	...
	err := forEachNode(kvBackend, func(_ kvdb.RTx,
		...
		// Check here if the node has opaque addr that included DNS
		// addresses type. If it does, it means it failed
		...
		for _, addr := range node.Addresses {
			...
			// TODO: rather export the DNS address type in lnwire
			// and check against that instead of hardcoding the
			// value 0x05.
