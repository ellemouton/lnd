...
func createTestVertex() (*models.LightningNode, error) {
...
func TestDisconnectBlockAtHeight(t *testing.T) {
	...
	sourceNode, err := createTestVertex()
...
func newEdgePolicy(chanID uint64,
...
func TestGraphTraversalCacheable(t *testing.T) {
	...
	err = graph.ForEachNodeCacheable(func(vertex route.Vertex,
		_ *lnwire.FeatureVector) error {

		nodeMap[vertex] = struct{}{}
	...
	for _, node := range nodes {
		err := graph.ForEachNodeChannel(node,
...
func assertNumNodes(t *testing.T, graph *ChannelGraph, n int) {
	numNodes := 0
	err := graph.ForEachNodeCacheable(func(_ route.Vertex,
		_ *lnwire.FeatureVector) error {
...
func TestChannelEdgePruningUpdateIndexDeletion(t *testing.T) {
	...
	db, ok := graph.V1Store.(*V1KVStore)
	if !ok {
		t.Skipf("skipping test for non-kvdb store")
	}
...
func TestNodeIsPublic(t *testing.T) {
	...
	require.NoError(t, err, "unable to make test database")
	aliceNode, err := createTestVertex()
	require.NoError(t, err, "unable to create test node")
	...
	require.NoError(t, err, "unable to make test database")
	carolNode, err := createTestVertex()
	require.NoError(t, err, "unable to create test node")
...
func TestEdgePolicyMissingMaxHtcl(t *testing.T) {
	...
	db, ok := graph.V1Store.(*V1KVStore)
	if !ok {
		t.Skipf("skipping test as it requires a V1KVStore")
	}
...
func TestLightningNodeSigVerification(t *testing.T) {
	...
	node, err := createLightningNode(priv)
