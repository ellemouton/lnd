...
func TestWakeUpOnStaleBranch(t *testing.T) {
	...
	require.NoError(t, err)
	edge1.FundingScript = fn.Some(fundingScript1)
	...
	require.NoError(t, err)
	edge2.FundingScript = fn.Some(fundingScript2)
	...
	_, _, has, isZombie, err := ctx.graph.HasV1ChannelEdge(chanID1)
	if err != nil {
	...
	_, _, has, isZombie, err = ctx.graph.HasV1ChannelEdge(chanID2)
	if err != nil {
...
func TestDisconnectedBlocks(t *testing.T) {
	...
	require.NoError(t, err)
	edge1.FundingScript = fn.Some([]byte{})
	...
	require.NoError(t, err)
	edge2.FundingScript = fn.Some([]byte{})
...
func TestChansClosedOfflinePruneGraph(t *testing.T) {
	...
	require.NoError(t, err)
	edge1.FundingScript = fn.Some(script)
	if err := ctx.builder.AddEdge(ctxb, edge1); err != nil {
		t.Fatalf("unable to add edge: %v", err)
	}

	// The router should now be aware of the channel we created above.
	_, _, hasChan, isZombie, err := ctx.graph.HasV1ChannelEdge(
		chanID1.ToUint64(),
	)
	if err != nil {
		t.Fatalf("error looking for edge: %v", chanID1)
...
func assertChannelsPruned(t *testing.T, graph *graphdb.VersionedGraph,
	...
	for _, channel := range channels {
		...
		_, _, exists, isZombie, err := graph.HasV1ChannelEdge(
			channel.ChannelID,
		)
		if err != nil {
