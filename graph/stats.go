...
// graphStats is a struct that tracks various updates to the graph and
...
func (g *graphStats) incNumEdgesDiscovered() {
	g.mu.Lock()
	g.numChannels++
	g.mu.Unlock()
}
...
func (g *graphStats) incNumChannelUpdates() {
	g.mu.Lock()
	g.numUpdates++
	g.mu.Unlock()
}
...
func (g *graphStats) incNumNodeUpdates() {
	g.mu.Lock()
	g.numNodes++
	g.mu.Unlock()
}
