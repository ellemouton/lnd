...
func TestReplyChannelRangeEncode(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := WriteMessage(w, replyMsg, 0)
...
func TestReplyChannelRangeDecode(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, id := range replyMsg.ShortChanIDs {
				scids[id.String()] = true
			}
			require.Len(t, scids, len(test.expSCIDs))

			for _, id := range test.expSCIDs {
				require.True(t, scids[id])
			}
